(() => {
  ...
  function buildProjectImage(imageUrl, baseUrl) {
    ...
    return `${baseUrl}images/projets/${source.replace(/^\/+/, "")}`;
  }

  function renderProjectCard(project, baseUrl, canFavorite) {
    const projectId = Number(project.id) || 0;
    const image = buildProjectImage(project.image_url, baseUrl);
    const title = escapeHtml(project.title || "Sans titre");
    ...
    const tags = String(project.technologies || "")
      ...
      .join("");
    const categorie = escapeHtml(project.categorie || "Autre");
    const isFavori = Boolean(project.is_favori);
    ...
    return `
      <div class="project-card"
           ...
           data-projet-open-modal="${projectId}"
           role="button"
           tabindex="0"
           aria-haspopup="dialog"
           aria-controls="modal-projet-${projectId}"
           aria-label="Ouvrir les informations du projet ${title}">
  ...
  function renderProjectModal(project, baseUrl, canFavorite) {
    const projectId = Number(project.id) || 0;
    const image = buildProjectImage(project.image_url, baseUrl);
    const title = escapeHtml(project.title || "Sans titre");
    const description = escapeHtml(project.description || "");
    const detailsUrl = `?page=projet&id=${encodeURIComponent(projectId)}`;
    const tags = String(project.technologies || "")
      .split(",")
      .map((tag) => tag.trim())
      .filter(Boolean)
    ...
    return `
      <div class="modal modal-detail modal-projet-dynamic"
           id="modal-projet-${projectId}"
           role="dialog"
           aria-modal="true"
           aria-hidden="true"
        ...
        <div class="modal-content">
          <div class="modal-header">
            ...
            <div class="modal-header-actions">
              ${favoriteButton}
              <button type="button" class="close-btn" data-projet-close-modal="${projectId}" aria-label="Fermer la modale">&times;</button>
            </div>
          </div>
                ...
                ${
                  tags
                    ? `<div class="modal-section-title">
                         <i class="fas fa-microchip"></i> Technologies utilisees
                       </div>
  ...
  function ensureProjectModals(projects, baseUrl, canFavorite) {
    document.querySelectorAll(".modal-projet-dynamic").forEach((modal) => modal.remove());
    ...
    projects.forEach((project) => {
      ...
      if (document.getElementById(`modal-projet-${projectId}`)) {
        return;
      }
      document.body.insertAdjacentHTML(
        "beforeend",
        renderProjectModal(project, baseUrl, canFavorite)
      );
      const modal = document.getElementById(`modal-projet-${projectId}`);
      if (modal) {
  ...
  function renderProjects(projects, baseUrl, canFavorite) {
    if (!projects.length) {
      return `
    ...
    return projects.map((project) => renderProjectCard(project, baseUrl, canFavorite)).join("");
  }
  ...
  function buildFilterUrl(q, categorie) {
    const params = new URLSearchParams({ page: "projets" });
    if (String(q || "").trim() !== "") {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    const filterForm = document.getElementById("projetsFilterForm");
    const input = document.getElementById("searchInput");
    const select = document.getElementById("categoryFilter");
    const resetButton = document.getElementById("resetFilters");
    const tagsContainer = document.getElementById("projetsFilterTags");
    const grid = document.getElementById("projetsGrid");
    const resultsText = document.getElementById("projetsResultsText");

    if (!input || !select || !grid) {
      return;
    }
    if (typeof AjaxHelper === "undefined") {
      console.error("AjaxHelper indisponible pour les projets.");
      return;
    }
    ...
    function mettreAJourUrl(filtres, pageCourante = 1) {
      const p = new URLSearchParams({ page: "projets" });
    ...
    async function requestAndRender(pageCourante = 1, options = {}) {
      ...
      const params = new URLSearchParams({ page: "projets", ajax: "1" });
      ...
      try {
        ...
        const totalFiltres = Number(payloadPagination?.total || projects.length || 0);

        grid.innerHTML = renderProjects(projects, baseUrl, canFavorite);
        initialiserFallbackImages(grid);
        ensureProjectModals(projects, baseUrl, canFavorite);
        renderCategories(select, categories, activeCat);
        renderCategoryTags(tagsContainer, categories, activeCat, activeQ);
        mettreAJourCompteurRecherche(projects.length, totalFiltres, activeQ, activeCat);
        ...
        if (typeof PublicNotification !== "undefined") {
          PublicNotification.erreur("Erreur lors du filtrage des projets.");
        } else if (typeof ToastNotification !== "undefined") {
          ToastNotification.erreur("Erreur lors du filtrage des projets.");
        }
      }
    }
    ...
    window.addEventListener("catalog:projet-created", () => {
      requestAndRender(1);
    });
